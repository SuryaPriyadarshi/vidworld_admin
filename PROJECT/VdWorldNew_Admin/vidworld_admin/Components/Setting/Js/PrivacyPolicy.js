var PrivacyPolicyApp = angular.module("rootApp.PrivacyPolicyApp", []);
PrivacyPolicyApp.controller("PrivacyPolicyController", ['$scope', '$rootScope', '$state', 'SweetAlert', 'APIInterface', 'AppConstant',
function ($scope, $rootScope, $state, SweetAlert, APIInterface, AppConstant) {
    ...
    function showUserTerms() {
        ...
        APIInterface.getDataWithAnyId(EndPoint.Admin, 'getPrivacyPolicy', 'user', function (response) {
            LoaderStart();
            if (response.Success) {
               // console.log(response)
                userterms = response.Result[0].PrivacyPolicy;
    ...
    function showCelebrityTerms() {
        ...
        APIInterface.getDataWithAnyId(EndPoint.Admin, 'getPrivacyPolicy', 'celebrity', function (response) {
            LoaderStart();
            if (response.Success) {
               // console.log(response)
                Celebrityterms = response.Result[0].PrivacyPolicy;
    ...
    function showReviewerTerms() {
        ...
        APIInterface.getDataWithAnyId(EndPoint.Admin, 'getPrivacyPolicy', 'reviewer', function (response) {
            LoaderStart();
            if (response.Success) {
              //  console.log(response)
                Reviewerterms = response.Result[0].PrivacyPolicy;
    ...
    function saveUserTerms() {
       ...
       // console.log(userterms);
        if (userterms == "" || userterms == null) {
            ...
            var postData = {
                UserType: 'user',
                PrivacyPolicy: userterms
            }
            LoaderStart();
            APIInterface.putData(EndPoint.Admin, 'privacyPolicy', postData, function (response) {
                if (response.Success) {
                    //    console.log(response);
                    LoaderStop();
                } else {
